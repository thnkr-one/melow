class Accounts::TransfersController < ApplicationController
  ...
  def create
    ...
    @transfer = Account::Transfer.build_from_accounts from_account, to_account, \
                                                      date: transfer_params[:date], amount: transfer_params[:amount].to_d,
                                                      currency: transfer_params[:currency], name: transfer_params[:name]
    if @transfer.save
      @transfer.entries.each(&:sync_account_later)
      redirect_to transactions_path, notice: "success"
    else
  ...
  def destroy
    ...
    redirect_back_or_to transactions_url, notice: "success"
  end
