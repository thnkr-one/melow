class UpgradesController < ApplicationController
  ...
  def acknowledge
    ...
    if upgrade
      if upgrade.available?
        ...
        flash[:notice] = "upgrade_dismissed"
      elsif upgrade.complete?
        Current.user.acknowledge_upgrade_alert(upgrade.commit_sha)
        flash[:notice] = "upgrade_complete_dismiss"
      else
        flash[:alert] = "upgrade_not_available"
      end
    else
      flash[:alert] = "upgrade_not_found"
    end
  ...
  def deploy
    ...
    # acknowledge the upgrade prompt
    Current.user.acknowledge_upgrade_prompt(upgrade.commit_sha)
