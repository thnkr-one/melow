module ApplicationHelper
  ...
  def format_money(number_or_money, options = {})
    unless number_or_money.is_a?(Money)
      Rails.logger.error "Unexpected value for money formatting: #{number_or_money.inspect}"
      return "N/A" # or some default value if `number_or_money` isn't what you expect
    end

    money = number_or_money
    number_to_currency(money.amount, unit: money.currency.symbol, precision: options.fetch(:precision, 2))
  end
